...
def update_movie(movie_id: int, movie_data: MovieCreate):
    for movie in movies_db:
        if movie.id == movie_id:
            ...
            movie.available = movie_data.available
            return movie
    raise HTTPException(status_code=404, detail="Movie not found")
...
@app.get("/movies/available/{status}")
def segun_disponibilidad(status: bool):
    listaPeliculas:List[Movie] = []
    for movie in movies_db:
        if movie.available == status:
            listaPeliculas.append(movie)
    return listaPeliculas
    
@app.get("/movies/director/{director_name}")
def segun_director(director_name: str):
    listaPeliculas:List[Movie] = []
    for movie in movies_db:
